...
import UserChecklistDetailsDatatable from "./UserChecklistDetailsDataTable";
...
import UserChecklistDetailsFakerDialogComponent from "./UserChecklistDetailsFakerDialogComponent";
import UserChecklistDetailsSeederDialogComponent from "./UserChecklistDetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const UserChecklistDetailsPage = (props) => {
    ...
    const filename = "userChecklistDetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("userChecklistDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "UserChecklistDetails", type: "error", message: error.message || "Failed get UserChecklistDetails" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("userChecklistDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "UserChecklistDetails", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("userChecklistDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "UserChecklistDetails",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/userChecklistDetails/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>UserChecklistDetails </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="userChecklistDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="userChecklistDetails-datatable">
                    <UserChecklistDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <UserChecklistDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <UserChecklistDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <UserChecklistDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <UserChecklistDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(UserChecklistDetailsPage);

...
import userChecklistDetailsFakerFactory from "./userChecklistDetailsFakerFactory";
...
const UserChecklistDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userChecklistDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userChecklistDetails").create(elem));
...
export default connect(mapState, mapDispatch)(UserChecklistDetailsFakerDialogComponent);

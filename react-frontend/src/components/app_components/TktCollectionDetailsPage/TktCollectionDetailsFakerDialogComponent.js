...
import tktCollectionDetailsFakerFactory from "./tktCollectionDetailsFakerFactory";
...
const TktCollectionDetailsFakerDialogComponent = (props) => {
...
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = tktCollectionDetailsFakerFactory(props.user,fakerCount,userNameIds,userMailAddressIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tktCollectionDetails").create(elem));
...
export default connect(mapState, mapDispatch)(TktCollectionDetailsFakerDialogComponent);

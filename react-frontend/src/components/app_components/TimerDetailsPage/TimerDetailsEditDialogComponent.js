...
staffId: _entity?.staffId,
startTime: _entity?.startTime,
endTime: _entity?.endTime,
timerStatus: _entity?.timerStatus,
        };

        setLoading(true);
        try {
            
        const result = await client.service("timerDetails").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info timerDetails updated successfully" });
    ...
    return (
        <Dialog header="Edit TimerDetails" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="timerDetails-edit-dialog-component">

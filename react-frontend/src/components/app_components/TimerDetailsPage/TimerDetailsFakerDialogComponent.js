...
import timerDetailsFakerFactory from "./timerDetailsFakerFactory";
...
const TimerDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = timerDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("timerDetails").create(elem));
...
export default connect(mapState, mapDispatch)(TimerDetailsFakerDialogComponent);
